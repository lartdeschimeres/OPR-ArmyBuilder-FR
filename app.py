...
if st.session_state.page == "faction_select":
        ...
        try:
            ...
            if isinstance(saved_lists, list):
                for i, saved_list in enumerate(saved_lists):
                    ...
                    if st.button(f"Charger la liste {i}", key=f"load_list_{i}"):
    ...
    if st.button("Créer une nouvelle liste", key="create_list_unique"):
...
# PAGE 2 - Liste des unités (rectangles cliquables)
    ...
    with col1:
        if st.button("⬅ Retour", key="back_to_faction_select_unique"):
            st.session_state.page = "faction_select"
            st.rerun()
    ...
    with col1:
        if st.button("Sauvegarder", key="save_list_unique"):
            saved_lists = ls_get("opr_saved_lists")
...
# PAGE 3 - Options de l'unité (avec radio buttons pour les héros)
    ...
    # Bouton retour
    if st.button("⬅ Retour à la liste", key="back_to_army_builder_unique"):
        st.session_state.page = "army_builder"
        st.rerun()
    ...
    if unit.get('type').lower() != 'hero':
        options['combined'] = st.checkbox("Unité combinée", value=options.get('combined', False), key="combined_unit_unique")
            ...
            if group['type'] == 'mount':
                if unit.get('type').lower() == 'hero':
                    ...
                    selected_mount = st.radio(
                        ...
                        index=0,
                        key=f"mount_radio_{group['group']}_unique"
                    )
                ...
                if unit.get('type').lower() == 'hero':
                    ...
                    selected_upgrade = st.radio(
                        ...
                        index=0,
                        key=f"upgrade_radio_{group['group']}_unique"
                    )
